...
plt1 = ggplot(df, aes(x = Mean, y = STD, col=Year)) + scale_color_viridis_c() + 
  ...
  theme_bw() +
  labs(title = expression("Tree Cover: " * mu * " versus " * sigma),
...
p1 = plt1 + geom_line(data=thdf,aes(x=x,y=y),color="red",linewidth=2) + xlim(0,0.8) + ylim(0,0.55) + 
  ...
  annotate("text",x=0.1, y=0.5,label=paste("MCH=",as.character(round(0.3276948,2)),sep=""),color="blue",size=6)

#p1

#ggsave("/Users/9oy/Documents/Projects/DSFResearch/Paper1/Figures/TCFig1_380MSAsv1.jpg",p1)
...
p2 = plt2 + geom_line(data=thdf,aes(x=x,y=y),color="red",linewidth=2,alpha=0.5) + xlim(0,0.8) + ylim(0,0.55) + 
  ...
  annotate("text",x=0.6, y=0.2,label=paste("MCH=",as.character(round(0.7318113,2)),sep=""),color="blue",size=6)

p2
ggsave("/Users/9oy/Documents/Projects/DSFResearch/Paper1/Figures/TCFig2_380MSAs_30m.jpg",p2)
...
p3.1 = p3 + geom_histogram(data=df30 %>% filter(Year==1985),alpha=0.5,fill="blue") + 
  ...
  annotate("text",x=0.55, y=900,label=paste("MCH-1985=","0.70",sep=""),color="blue",size=6) + 
  annotate("text",x=0.55, y=800,label=paste("MCH-2023=",as.character(round(MCHm23,2)),sep=""),color="red",size=6)
p3.1
ggsave("/Users/9oy/Documents/Projects/DSFResearch/Paper1/Figures/TCFig3_380MSAs_30m_hist.jpg",p3.1)
...
ggsave("/Users/9oy/Documents/Projects/DSFResearch/Paper1/Figures/TCFig4_380MSAs_30m_Change.jpg",p4.1)

sum(out$tau > 0)
sum(out$sslope > 0)

LA = df %>% filter(NAME=="Los Angeles-Long Beach-Anaheim, CA") %>% filter(Year%in%c(1985,2023))
