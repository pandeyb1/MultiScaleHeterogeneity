...
class UrbanGrowthModel:
    ...
    """
    def __init__(self, grid_size=200, neighborhood_size=10, contagion_factor=0.5):
        """
    ...
    def run_simulation(self, n_steps=None,seed=None):
        ...
        for step in range(n_steps):
            ...
            if step == (n_steps - 1):
                ...
                mi_neigh = self._calculate_neighborhood_moran()
        
        return np.array(means), np.array(sds), mi, mi_neigh
    ...
    def _calculate_neighborhood_moran(self):
        """
        NEW FUNCTION: Calculates Moran's I on the aggregated neighborhood means.
        """
        n_size = self.neighborhood_size
        
        # 1. Aggregate grid into neighborhood means (Same logic as _calculate_stats)
        # Result shape will be (10, 10) if grid is 100x100 and n_size is 10
        neighborhoods = self.grid.reshape(self.grid_size // n_size, n_size, 
                                           self.grid_size // n_size, n_size)
        neighborhoods = neighborhoods.transpose(0, 2, 1, 3)
        neighborhood_means = np.mean(neighborhoods, axis=(2, 3))
        
        # 2. Create Weights Matrix for the coarser grid dimensions
        # If grid is 100 and neighborhood is 10, dimensions are 10x10
        h, w_dim = neighborhood_means.shape
        w = lat2W(h, w_dim)
        
        # 3. Calculate Moran's I on the means
        mi = Moran(neighborhood_means, w).I
        return mi
...
means_low, sds_low,mi, mi_neigh = model_low.run_simulation(seed=localsimulationseed)
alpha_low = model_low.analyze_alpha(means_low, sds_low)
print(f"Estimated Alpha for low contagion: {alpha_low:.4f}")
print(f"Estimated Morans I for low contagion: {mi:.4f}")
print(f"Estimated Neighborhood Morans I for low contagion: {mi_neigh:.4f}")
...
means_high, sds_high,mi,mi_neigh = model_high.run_simulation(seed=localsimulationseed)
...
print(f"Estimated Neighborhood Morans I for high contagion: {mi_neigh:.4f}")
...
def process_chunk(cont_chunk, seed_chunk):
    ...
    mis_chunk_neigh = np.zeros(len(cont_chunk))
    
    for i, (cont, seed) in enumerate(zip(cont_chunk, seed_chunk)):
        model = UrbanGrowthModel(contagion_factor=cont)
        means, sds, mi, mi_neigh = model.run_simulation(seed=int(seed))  # Ensure seed is int
        alpha = model.analyze_alpha(means, sds)
        alphas_chunk[i] = alpha
        mis_chunk[i] = mi
        mis_chunk_neigh[i] = mi_neigh

    return alphas_chunk, mis_chunk, mis_chunk_neigh
...
alphas_list, mis_list,mis_neigh_list = zip(*results)
alphas = np.concatenate(alphas_list)
mis = np.concatenate(mis_list)
mis_neigh = np.concatenate(mis_neigh_list)
...
plt.style.use('default')
plt.scatter(mis_neigh,alphas)
plt.xlabel("Moran's I Neighborhood")
plt.ylabel("Mean Conditional Heterogeneity")
